...
def time_schedule_clients():

    #################################
    global initialized,time_slice
    while not initialized:  # 等待初始化
        pass
    ##################################
...
def priority_schedule_clients():

    #################################
    global initialized,interval
    while not initialized:  # 等待初始化
        pass
    ##################################
...
def initialize():
    global initialized, y, interval, time_slice
    msg = request.get_json()
    print("msg of start:", msg)
    y = int(str(msg['defaultRoomNum']))
    interval = float(str(msg['avoidTime']))
    time_slice = int(str(msg['timeSlice']))
    initialized = True
    return 'ok'
...
def get_op_request():
    ...
    ############################
    global initialized, interval, time_slice
    if not initialized:
        return 'op not initialized'
    #############################
    ...
    #now_tp = int(request.headers["now_temp"])
    ...
    if room_id not in client_dict:
        print("room_id:", room_id, " joins in, the wind speed is set as ",
              wind_speed)
        dftmp = now_tp
        newclient = client(room_id, wind_speed, target_tp, mode,now_tp, dftmp)
        client_dict[room_id] = newclient  #新来的房间上来默认先等待
        ##################################################
        newclient.interval = interval  #给client中的interval赋值
        newclient.sec = time_slice / 2
        ##################################################
    else:
...
if __name__ == '__main__':
    #y = int(input("y is:"))
    scheduler = threading.Thread(target=priority_schedule_clients)
    scheduler2 = threading.Thread(target=time_schedule_clients)
    scheduler.start()
    scheduler2.start()
